...
def searchShoppings():
  global creds, selected_shop_list_keys, select_var, select_shop_list_key, root, messages, total_tickets_label
...
def pick_date(event, day, date_entry):
  ...
  submit_btn = Button(date_window, text="Submit", command= lambda d=day, de=date_entry: grab_date(d, de))
  submit_btn.place(x=80, y=190)
  delete_window = date_window

def grab_date(day, de):
  ...
  de.delete(0, END)
  de.insert(0, f"{day.strftime('%Y/%m/%d')}")
...
def main():
  ...
  date_from_value.bind("<1>", lambda event, day=from_day, day_entry=date_from_value: pick_date(event, day, day_entry))
  ...
  date_to_value.bind("<1>", lambda event, day=to_day, day_entry=date_to_value: pick_date(event, day, day_entry))
